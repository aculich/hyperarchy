...
describe "GET /fetch_organization_page", :type => :rack do
  ...
  it "returns elections, candidates, and election visits from the highest fetched page to the next page" do
    # must be logged in
    get "/fetch_organization_page", :organization_id => organization.id, :page => 3, :highest_fetched_page => 1, :items_per_page => 2
    last_response.body.should include("/login?redirected_from=") # soft redirect
